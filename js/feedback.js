jQuery(document).ready(function($){ 
	var post_id;
	$(".post_feedback_button").live("click", function() {
	 	$(".feedback_panel").fadeIn(300);
	 	 post_id = this.id;
	});
	var comment_id;
	$(".comment_feedback_button").live("click", function() {
	 	$(".comment_feedback_panel").fadeIn(300);
	 	comment_id = this.id;
	});
	...
	$(".save_comment_feedback_btn").click(function() {
		...
		if(agree == 0 && disagree == 0 && relevant == 0 && informative == 0 && well_written == 0 && unserious == 0) {
			...
			$.ajax ({
				url : siteURL + "/feedback/validate_comment_feedback",
				type : 'POST',
				data : data,
				success : function (data) {
					data = $.parseJSON(data);
					if(data.response == "ok") {
						alert(data.msg);
						$("#lightbox, .comment_feedback_panel").fadeOut(300);
					} else {
						alert(data.error);
					}
				}	
	...
	$(".close_comment_feedback_btn").click(function() { 
	 	$(".comment_feedback_panel").fadeOut(300);
	});
